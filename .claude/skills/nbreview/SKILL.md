---
name: nbreview
description: This skill should be used when the user asks to "review this notebook", "nbreview", "check my notebook", or otherwise requests a Jupyter notebook review. Produces a findings-only report on a single `.ipynb` file.
---

# Jupyter Notebook Review

Review a single Jupyter notebook.

## Before reviewing

- The path is usually provided as an argument. If not, guess the most likely notebook (e.g. the one modified in `git status`, or the only `.ipynb` in the directory) and ask the user to confirm before starting the review.
...
### Header metadata (first markdown cell)
...
- "Important" is a judgment call, not a checklist. A useful test: would changing this value alone produce a different, still-sensible run of the notebook? If yes, it probably belongs up top; if it only makes sense alongside edits to a specific cell's logic, it can live where it's used. Variables defined mid-notebook at their point of use are **fine** — only flag when an important knob is hidden in a way that forces a reconfigurer to hunt for it, or when the same value is hard-coded in several places that would need to change together.
- If the notebook uses randomness in computations that feed results, a **seed** is one of the important knobs and its effect is noted.

### Structure

- A short markdown cell precedes each code cell
  - Markdown opens with a human-readable one-liner. If needed, extended content goes under a bold **Details:** label so readers can skim.
...
- Functions longer than a few lines, or reused across notebooks, should live in a `.py` module rather than being defined (or copy-pasted) inline.
- Paths are relative to the notebook or held in a config variable — no absolute paths like `/Users/you/…`.

### Data integrity

- **Assertions at transformation boundaries.** After each meaningful data transformation (load, clean, filter, join, aggregate), an `assert` cell checks the invariants that the next stage assumes — expected columns, dtypes, non-null keys, value ranges. Silent data corruption is the failure mode these catch.
- **Join row counts.** After a join, an assertion checks that the row count is within ~5% of the expected count (typically the left side's row count for a left join, or a known multiplier for one-to-many). An unexpected blow-up or shrink almost always means a key mismatch or duplicate keys and should fail loudly rather than propagate.
- **Idempotency discipline.** Any cell that modifies a shared object (DataFrame, file, database) should be safe to re-run — prefer rebuilding from source (`df = df_raw.drop(...)`) over in-place mutation of a name from an earlier cell, or guard the mutation so the second run is a no-op.
...
- Group findings by the section headings above.
...
- **Trace each fix forward through the notebook before proposing or applying it.** A fix that looks locally correct can break a later cell that depends on the changed name, type, shape, or ordering. Walk through every cell that follows — and through any sibling notebook that imports the same module or reads the same output — before committing to the change.
- **Never rename a column without checking every downstream reference.** Search the rest of the notebook, the other notebooks in the directory, and any `.py` modules for the old name before renaming. If references exist, either update all of them in the same change or leave the name alone.
